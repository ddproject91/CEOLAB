...
export function rowLabel(row: Row, index: number): string {
  const candidate =
    row.title ?? row.name ?? row.nameKo ?? row.label ?? row.id ?? row.slug;
    ...
    <div>
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {message && (
            <span
              className={`text-xs font-medium ${message.type === "ok" ? "text-emerald-600" : "text-red-600"}`}
            >
              {message.text}
            </span>
          )}
          <button
            type="button"
            onClick={addItem}
            className="rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-medium hover:border-gray-400"
          >
            + 항목 추가
          </button>
          <button
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-[220px_1fr]">
        <div className="max-h-[70vh] overflow-y-auto rounded-xl border border-gray-200">
          {items.map((row, index) => (
            <div
              key={index}
              className={`flex items-center justify-between border-b border-gray-200 px-3 py-2 text-xs last:border-b-0 ${
                selected === index ? "bg-[#F3E9C7]/60 text-[#8A6A16]" : "hover:bg-gray-50"
              }`}
            >
              <button
                type="button"
              ...
              <div className="flex shrink-0 items-center gap-1">
                <button
                  ...
                  className="rounded px-1 text-gray-400 hover:text-gray-900"
                >
                  ⧉
                </button>
                <button
          ...
          {items.length === 0 && (
            <p className="px-3 py-4 text-center text-xs text-gray-400">항목이 없습니다.</p>
          )}
        </div>
        ...
        <div className="rounded-xl border border-gray-200 p-4">
          {current && selected != null ? (
            ...
            <p className="text-sm text-gray-400">왼쪽에서 항목을 선택하세요.</p>
          )}
        </div>
      </div>
    </div>
...
export function FieldInput({
  ...
  return (
    <label className="flex flex-col gap-1">
      <span className="text-[11px] font-medium text-gray-500">{field.key}</span>
      {(field.type === "string" || (isImageUrlField && field.type === "text")) && (
        <div className="flex items-center gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:border-[#C9A227]"
          />
          {isImageUrlField && (
            <label className="shrink-0 cursor-pointer rounded-lg border border-gray-300 px-3 py-2 text-xs font-medium hover:border-[#C9A227]">
      ...
      {isImageUrlField && typeof value === "string" && value && (
        ...
        <img
          src={value}
          alt=""
          className="mt-1 h-20 w-20 rounded-lg border border-gray-200 object-cover"
        />
      )}
      {field.type === "text" && !isImageUrlField && (
        <textarea
          ...
          onChange={(e) => onChange(e.target.value)}
          rows={4}
          className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:border-[#C9A227]"
        />
      )}
      {field.type === "number" && (
        <input
      ...
      {field.type === "json" && isImageArrayField && (
        <div className="flex flex-col gap-2">
          <div className="flex flex-wrap gap-2">
            ...
            <label className="flex h-16 w-16 cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-gray-300 text-center text-[10px] text-gray-400 hover:border-[#C9A227]">
              {uploading ? "업로드중" : "+ 추가"}
              <input
                type="file"
                accept="image/*"
      ...
      {field.type === "json" && !isImageArrayField && (
        <>
          <textarea
            ...
            rows={6}
            className="rounded-lg border border-gray-300 bg-white px-3 py-2 font-mono text-xs outline-none focus:border-[#C9A227]"
          />
          {jsonError && <span className="text-[11px] text-red-600">{jsonError}</span>}
        </>
      )}
    </label>
  );
